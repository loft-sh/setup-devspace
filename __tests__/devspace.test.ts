...
test('binaryUrl(platform: string, architecture: string, version: string)', async () => {
  await expect(binaryUrl('darwin', 'arm64', '5.14.4')).resolves.toEqual(
    'https://github.com/devspace-sh/devspace/releases/download/v5.14.4/devspace-darwin-arm64'
  )
  await expect(binaryUrl('darwin', 'amd64', '5.14.4')).resolves.toEqual(
    'https://github.com/devspace-sh/devspace/releases/download/v5.14.4/devspace-darwin-amd64'
  )
  ...
  await expect(binaryUrl('linux', 'arm64', '5.14.4')).resolves.toEqual(
    'https://github.com/devspace-sh/devspace/releases/download/v5.14.4/devspace-linux-arm64'
  )
  await expect(binaryUrl('linux', 'amd64', '5.14.4')).resolves.toEqual(
    'https://github.com/devspace-sh/devspace/releases/download/v5.14.4/devspace-linux-amd64'
  )
  ...
  await expect(binaryUrl('win32', 'amd64', '5.14.4')).resolves.toEqual(
    'https://github.com/devspace-sh/devspace/releases/download/v5.14.4/devspace-windows-amd64.exe'
  )
